...
{
    ByteMatrix tempState(4, ByteArray(4, 0));
    ...
    for (size_t i = 0; i < 4; ++i)
    {
        for (size_t k = 0; k < 4; ++k)
        {
            for (size_t j = 0; j < 4; ++j)
            {
                if (CMDS[i][k] == 1)
                {
                    tempState[i][j] = tempState[i][j] ^ state[k][j];
                }
                else
                {
                    tempState[i][j] = tempState[i][j] ^ GF_MUL_TABLE[CMDS[i][k]][state[k][j]];
                }
            }
        }
    }

    for (size_t i = 0; i < 4; ++i) state[i] = tempState[i];
}
